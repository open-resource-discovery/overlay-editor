...
import { Button, SplitPane } from "@open-resource-discovery/ui-components";
import { JsonEditor } from "@/components/playground/JsonEditor";
import { OverlayCardView } from "@/src/lib/card-view";
import {
  loadOverlayBody,
  loadPredefinedOverlays,
  type PredefinedOverlay,
} from "@/lib/playground/predefined-overlays";
import { OverlaySelector } from "@/components/playground/OverlaySelector";
...
// `number | string` and treats bare numbers as **pixels** — so the only way
// to express percentages is a string like "22%". We pass strings at runtime
// (the right thing) and erase the type narrowing here. Drop this helper once
// `@open-resource-discovery/ui-components` widens its prop types.
const pct = (s: string): number => s as unknown as number;

export function Playground() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const requestedId = searchParams.get(QUERY_PARAM);
  ...
  useEffect(() => {
    if (!url) return;
    let alive = true;
    // Mark loading via microtask so the lint rule doesn't flag this as a
      ...
      <SplitPane.Panel defaultSize={pct("40%")} minSize={pct("20%")}>
        <div className="relative h-full min-h-0 overflow-auto">
          {loading ? (
            ...
            <OverlayCardView content={content} />
          )}
        </div>
      </SplitPane.Panel>
    </SplitPane.Root>
